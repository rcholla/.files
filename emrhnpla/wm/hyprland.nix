...
let
  ...
  userpkgs = {
    ...
    hyprbin-gamemode = pkgs.writeShellApplication {
      name = "hyprbin-gamemode";
      text = ''
        hyprbin-effects toggle &
        hyprbin-waybar toggle &
        hyprbin-wallpaper toggle &
      '';
    };
    ...
    hyprbin-wallpaper = pkgs.writeShellApplication {
      ...
      text = ''
        ...
        function _finit { mpvpaper eDP-1 "${usercfg.wallpapers.desktop.path "mp4"}" -o "no-audio --loop-playlist"; }
...
in
{
  ...
  xdg.configFile."hypr/CatppuccinMocha.conf".source = "${userpkgs.catppuccin-hyprland}/themes/mocha.conf";
  wayland.windowManager.hyprland = {
    ...
    extraConfig = ''
      ...
      # # Source;
      source = $XDG_CONFIG_HOME/hypr/CatppuccinMocha.conf
      # # Exec;
      exec-once = swayidle -w timeout 600 "swaylock -f" timeout 3600 "hyprctl dispatch dpms off" resume "hyprctl dispatch dpms on"
      ...
      bind = $mainMod_CTRL, G, exec, hyprbin-gamemode
      ...
      bind = $mainMod_SHIFT, X, exec, swaylock -f
